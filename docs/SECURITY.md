...
- **Strict Content-Security-Policy, meta-delivered.** `default-src 'none'` with
  ...
  per-request nonces, the policy is computed at build time and injected as the
  first `<head>` child of every document — a `<meta>` CSP does not govern inline
  scripts that precede it. **Status: IMPLEMENTED (enforcing).** Generated by
...
- **Subresource Integrity (SRI).** `integrity=` on the entry ES module and both
  ...
  expressible on them — documented, not silently skipped. **Status: NOT YET
  IMPLEMENTED (Phase 3).**
