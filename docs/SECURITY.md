...
- **Strict Content-Security-Policy, meta-delivered.** `default-src 'none'` with
  ...
  `WebAssembly` on any page). Because GitHub Pages cannot set response headers or
  mint per-request nonces, the policy is computed at build time and injected
  ...
  every inline script. The policy is **uniform across every page** — there is no
  per-page relaxation. Gated by `tests/e2e/csp.spec.ts`, which loads every shell
  under the enforcing policy, asserts `script-src` is `'self'` + hashes only (no
  `unsafe-*`, no `wasm-unsafe-eval`, no hosts/schemes/wildcards), and asserts zero
