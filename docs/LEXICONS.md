...
_Last updated: 2026-07-16 (registered planned `app.arecipe.release` +
`app.arecipe.status` — signed-releases naming ruling; earlier same day:
cookFollow reconciling mirror + `publishedRkey` marker, live round-trip
harness `cook-follows-live.spec.ts`)._
