...
| `app.arecipe.mealPlan` | live | Weekly meal planner: `#week` (1–12 `repeat`, exactly 7 `#slot` days) → `#slot` (optional recipe `strongRef` + `note`, plus an open-world cached display `name`); calendar is a derived view, not stored. Caveat: a slot's `recipe` strongRef cannot declare its target is a recipe (same open-world caveat as consumed strongRefs) — the app treats it as a recipe ref by construction. Round-trip confirmed live 2026-07-10 (`LIVE=1 meals-live.spec.ts`). **Second consumer (2026-07-12): the iCalendar (.ics) meal-plan feed** — a scheduled GitHub Action reads a configured DID's public `mealPlan` records and renders them to a subscribable `.ics` calendar. **No new NSID is minted:** the feed is a read-only *consumer* of the existing record; the dated calendar it emits is the same derived view the app renders (shared `deriveDatedSlots`, anti-drift). See `plans/2026-07-12-1-plan-ics-meal-plan-feed.md`. | `src/recipes/meal-plan-sync.ts` (`MEAL_PLAN_COLLECTION`, `syncPlanToPds`/`listPdsPlans`); model `src/recipes/meal-plan.ts`; feed `src/recipes/ics-*.ts` + `scripts/build-ics-feed.mjs` |
