...
| `app.arecipe.mealPlan` | planned | Weekly meal planner: `#week` (1–12 `repeat`, exactly 7 `#slot` days) → `#slot` (optional recipe `strongRef` + `note`); calendar is a derived view, not stored. Caveat: a slot's `recipe` strongRef cannot declare its target is a recipe (same open-world caveat as consumed strongRefs) — the app treats it as a recipe ref by construction. Flips to `live` when Phase 9 writes real records. | `src/recipes/meal-plan.ts` (`MEAL_PLAN_COLLECTION`); fixture `tests/fixtures/lexicons/app.arecipe.mealPlan.json` |
