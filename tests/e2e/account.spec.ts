...
test('shopping prefs: substitutions add/remove, always-apply toggle persist (device-local)', async ({
  ...
  // An ingredient the vocabulary does not know is refused WITH WORDS — a rule
  // keyed on nothing could never match, so it is never stored (no chip).
  await page.getByTestId('substitution-from').fill('xyzzy powder');
  await page.getByTestId('substitution-to').fill('anything');
  await page.getByTestId('substitution-add').click();
  await expect(page.getByTestId('substitution-unknown')).toBeVisible();
  await expect(page.getByTestId('substitution-unknown')).toContainText('doesn’t know “xyzzy powder”');
  await expect(page.getByTestId('substitution-chip')).toHaveCount(1);
