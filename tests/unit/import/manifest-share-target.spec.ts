...
// share target on the Acquire hub (import.html) with the title/text/url params
// the importer reads. A GET target (data in the query string) needs no
// service-worker POST handler.
...
describe('manifest share_target', () => {
  ...
  it('registers a GET share target landing on the Acquire hub (import.html)', () => {
    expect(manifest.share_target).toBeDefined();
    expect(manifest.share_target?.action).toMatch(/import\.html$/);
