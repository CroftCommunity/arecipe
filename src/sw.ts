...
// Build ids whose snapshot directory must survive the activate-time purge
// (snapshot D5): the active build, plus the builds the release config protects —
// the pinned version (D4) and the last verified version (D3's enforcement
// fallback). Both are routing targets, so their snapshots must outlive version
// turnover exactly as their whole caches do (shouldDeleteCache).
const pinnedBuildIds = (cfg: ReleaseConfig): string[] =>
  [cfg.lockedVersion, cfg.lastVerifiedVersion].filter((v): v is string => v !== undefined);
...
sw.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      ...
      // neither the active build nor a protected build (snapshot D5) — pinned
      // and last-verified snapshots are never deleted.
      const cache = await caches.open(CACHE);
      const keep = new Set([__BUILD_VERSION__, ...pinnedBuildIds(cfg)]);
