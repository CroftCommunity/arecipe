...
// The date-sha version stays the display string; buildNumber (commit count)
// is the MONOTONIC counter the release manifest and the client-side
// regression check compare (signed releases D1).
...
const result = buildSync({
  ...
  outdir: 'dist',
  metafile: true,
  // __RELEASE_PUBKEY__ is the pinned release pubkey only — NOT buildNumber,
  // which changes every deploy; pages learn the running build's
  // version/buildNumber from the controlling SW instead.
  define: {
    __SNAPSHOT_BUILD__: JSON.stringify(version),
