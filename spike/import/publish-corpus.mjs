...
for (const [file, srcLabel] of FILES) {
  ...
  recipes.forEach((r, i) => {
    ...
    if (Array.isArray(r.methods) && r.methods.length > 0) {
      // Method siblings carry their own times when the method declares them —
      // master-pot slow-cooker vs pressure-cooker durations differ by an order of magnitude.
      for (const m of r.methods) versions.push({ base: r, dishKey, name: `${r.name} (${m.label})`, versionLabel: m.label, instructions: m.steps ?? [], prepTime: m.prepTime, cookTime: m.cookTime, totalTime: m.totalTime });
    } else {
...
const toRecord = (v) => {
  ...
  for (const t of ['prepTime', 'cookTime', 'totalTime']) if (v[t]) rec[t] = v[t];
